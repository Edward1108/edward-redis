 ...
 * Author: Edward1108 <xuyuanhua1987@163.com>
 * Date:2020/5/22
 * Time:15:45
 */
...
//        $this->handler->setOption(\Redis::OPT_PREFIX,$this->options['prefix']);
     ...
     * @return bool
     */
    public function mset($arr,$expire=null)
    {
        $new_arr = [];
        foreach ($arr as $key=>$value){
            $new_key = $this->getCacheKey($key);
            $new_arr[$new_key] = $value;
        }
        $result = $this->handler->mset($new_arr);
    ...
    /**
     * 删除指定key，可以多个
     * @param string|array $keys 删除单个key时$keys为字符串，多个key时为数组
     * @return int
     * @example
     * <pre>
     * $redis->del('key1');
    ...
    public function del($keys)
    {
        ...
        foreach ($key_arr as $key=>$value){
            $key_arr[$key] = $this->getCacheKey($value);
        }
        return $this->handler->del($key_arr);
    }
